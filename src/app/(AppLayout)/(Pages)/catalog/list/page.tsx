...
async function Page(props: Props) {
    ...
    const filterParams: Array<{
        ...
        {
            filter: "FuelType",
            value: FuelType.find((obj) => obj.en === fuel)?.ru
        },
        {
            filter: "Transmission",
    ...
    return (
        <Box className="mb-5">
            <div className="flex flex-col items-center">
                <h2 className="text-center">Каталог автомобилей с аукциона
                    Lotte</h2>
                <span className="text-sm opacity-70">
          (Информация обновляется еженедельно по пятницам)
        </span>
            </div>
            <section className="my-5 rounded-box bg-gray-100">
                <SearchFilter/>
            </section>
            <section
                className="grid grid-cols-1 justify-items-center gap-5 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
